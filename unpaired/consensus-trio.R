...
Mu <- read.delim(file = SAMfile, row.names=NULL, 
                header = FALSE, skip = 1)
annHead <- read.delim(file = SAMfile, row.names = NULL, 
                      nrows = 1, header=FALSE, stringsAsFactors = FALSE)
extraHead <- c("QUAL1", "DP.SAM", "CHR", "POS", "END", "REF", 
...
a <- gsub(".*:", "",Mu$MAGIC.SAM);
REF <- as.numeric(gsub(",.*$","",a));
Mu$AD.SAM <- as.numeric(gsub("^.*,","",a));
Mu$DP.SAM <- REF+Mu$AD.SAM;
Mu$VAF.SAM <- 100*Mu$AD.SAM/Mu$DP.SAM;
Mu <- Filter(function(y)!all(y == "."), Mu);
Mu[Mu == ""] <- NA
Mu <- Mu %>% mutate_if(is.factor, as.character);
