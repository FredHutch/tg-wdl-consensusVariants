...
extraHead <- c("QUAL1", "DP.GATK", "CHR", "POS", "dbSNP", "REF", 
...
commonCols <- c("CHR", "POS", "REF", "ALT", "ExonicFunc.refGene", "ExAC_ALL", "cosmic70")
GMerge <- G %>% select(all_of(c(commonCols, "AD.GATK", "DP.GATK")))
SMerge <- S %>% select(all_of(c(commonCols, "AD.SAM", "DP.SAM")))
MuMerge <- Mu %>% select(all_of(c(commonCols, "FILTER.Mu","AD.Mu","DP.Mu")))
